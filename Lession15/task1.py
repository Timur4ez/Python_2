...
def create_files_with_extension(extension, num_files=42, **kwargs):
    try:
        for _ in range(num_files):
            filename = generate_random_filename(length_min=kwargs.get('length_min', 6), length_max=kwargs.get('length_max', 30)) + f".{extension}"
            file_size = random.randint(kwargs.get('min_bytes', 256), kwargs.get('max_bytes', 4096))
            file_content = generate_random_bytes(min_bytes=kwargs.get('min_bytes', 256), max_bytes=kwargs.get('max_bytes', 4096))
